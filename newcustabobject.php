...
$abon_plata_customer = trim(filter_input(INPUT_POST, 'abon_plata_customer', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
...
if(isset($data['save_to_object']))
	{
		...
		if(empty($abon_plata_customer) OR $abon_plata_customer == 0)
		{
		...
		if(empty($errors))
		{  
			$result = Add_Object_customabont($link, $id_object, $abon_plata_customer, $year, $month, $date_payment_result, $payment_status, $pay_account ); 
		?>		
			<script>
...
<head>
	...
	<div class="showany">
	<p class="breadcrumbs"><a href='/object_customer_abon.php'>Объекты с абонентской платой</a> >  Добавление объекта с абонентской платой:</p>
	...
	<?php }?>	
		<form action="" method="POST">
				...
				<tr>
					...
					<td><input class="StyleSelectBox" id="abon_plata_customer" name="abon_plata_customer" step="any" type="number" min="0" value="<?=$abon_plata_customer;?>"/></td>
				</tr>
				<!-- НОМЕР СЧЕТА --> 
				<tr>
