...
$sheet->setCellValue('H6','Абонентская плата');
$sheet->setCellValue('I6','Статус оплаты');
$sheet->setCellValue('J6','Дата платежа');
$sheet->setCellValue('K6','Номер счета');
$row_start = 7;
$rowplus = 0;
...
foreach($_POST['id_contractors'] as $id_contractor)
{
	...
	if($objects){
		foreach($objects as $object)
		{
			...
			$sheet->setCellValue('H'.($row_next), $month);
			$sheet->setCellValue('I'.($row_next), $paymentstatus_array[$object['paystatus']]);
			$sheet->setCellValue('J'.($row_next), $object['paydate']);
			$sheet->setCellValue('K'.($row_next), $object['pay_account']);
...
/* ПРИМЕНЕНИЕ СТИЛЕЙ */
$sheet->getStyle('A6:K'.($row_next))->applyFromArray($style_wrap);
$sheet->getStyle('A6:K6')->applyFromArray($style_header);
...
$sheet->getStyle('A6:K6')->applyFromArray($style_center);
$sheet->getStyle('B1:B5')->applyFromArray($style_left);
$sheet->getStyle('A7:F'.($row_next))->applyFromArray($style_left);
$sheet->getStyle('F7:K'.($row_next+1))->applyFromArray($style_center);
