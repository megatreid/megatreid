...
if(isset($_SESSION['userlevel']) AND $_SESSION['userlevel']<=3)
{
	...
	if(isset($_GET['id_record']))
	{
		...
		$object_info = Edit_Object_with_abon($link, $id_record);
		$id_contractor = $object_info['id_contractor'];
		$id_object = $object_info['id_object'];
		$abon_plata_contr = $object_info['summ'];
		...
		$abon_plata_contr_update = trim(filter_input(INPUT_POST, 'abon_plata_contr', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
		...
		if(isset($data['save_to_object']))
		{
			...
			$object_exist = Object_Exist($link, $id_object_update, $year_update, $month_update);
			if(empty($id_object_update) OR $id_object_update == 0)
			{
				$errors[] = 'Выберите объект!';
			}
			if(empty($id_contractor_update) OR $id_contractor_update == 0)
			{
			...
			if(empty($abon_plata_contr_update) OR $abon_plata_contr_update == 0)
			{
...
<head>
	...
	<div class="showany">
	<p class="breadcrumbs"><a href='/object_contr_abon.php'>Объекты с абонентской платой</a> > Редактирование объекта с абонентской платой:</p>
	<div class="reg_sel_object">
	<p class="breadcrumbs"> Выбор объекта:</p>
	<?php if($err==TRUE){?>
		<div class="error-message"><?=array_shift($errors)?></div>
	<?php }?>	
		<form action="edit_object_contr_abon.php?id_record=<?=$id_record;?>" method="POST">
					<table>
						<tr>
				...
				<td>
					...
					<select name="city_id_contr" id="city_id_contr" class="StyleSelectBox">
						...
						<?php foreach($show_city_names as $i => $show_city_name)  { ?>
							<option  value="<?= $show_city_name['city_id']; ?>"<?= ($show_city_name['city_id'] == $contr_info['city_id']) ? 'selected' : ''?>><?= $show_city_name['name']; ?></option>
						<?php } ?>
					</select>
				</td>
				</tr>
				<tr id="contr_select">
					<td  class="rowt"><label for="id_contractor">Подрядчик:</label></td>
					<td>
						<select name="id_contractor" id="id_contractor"  class="StyleSelectBox">
							<option value="0">- Выберите подрядчика -</option>
							<option value="<?=$id_contractor;?>" selected><?=$contractor_info;?></option>
						</select>
					</td>
				</tr>
				<tr id="contr_select">
